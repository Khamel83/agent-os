...
// Global variable to track OpenRouter spending
let openRouterSpent = 0;
let openRouterBudget = 0; // This will be loaded from config
...
function loadConfig() {
    ...
    try {
        ...
        openRouterBudget = config.openrouter_budget_usd || 1.00; // Default to $1 if not set
        return config;
    } catch (error) {
        console.error("Error loading Claude Code Router config:", error);
        return {};
    }
}
...
async function getOpenRouterFreeModels() {
    ...
    if (!OPENROUTER_API_KEY) {
        console.warn("OPENROUTER_API_KEY not set. Cannot fetch models from OpenRouter.");
        return [];
    }
...
async function routeRequest(request) {
    loadConfig(); // Load config to get budget
    ...
    for (const model of rankedFreeModels) {
        // Check budget before attempting OpenRouter call
        if (openRouterSpent >= openRouterBudget) {
            console.warn(`OpenRouter budget of $${openRouterBudget} exceeded. Stopping OpenRouter calls.`);
            break; // Stop trying OpenRouter models
        }
        ...
        try {
            ...
            // Simulate a cost for tracking purposes, even if the model is "free"
            const simulatedCost = 0.001; // Example very small cost per call
            openRouterSpent += simulatedCost;

            return {
                model: model.id,
                response: `Response from ${model.name} (simulated)`,
                usage: { tokens: 50, cost: simulatedCost }
            };
        } catch (error) {
